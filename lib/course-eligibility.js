...
function isSelfPacedOrEvergreen(course = {}) {
  ...
  return /\b(evergreen|on[-\s]?demand|self[-\s]?paced|asynchronous|online ce|online course|release date|published)\b/.test(value);
}

export function prepareCourseForPublication(course = {}, today = getTodayISO()) {
  const startDate = dateOnly(course.start_date);
  const endDate = dateOnly(course.end_date);

  if (!startDate || startDate >= today || (endDate && endDate < today) || !isSelfPacedOrEvergreen(course)) {
    return course;
  }
  ...
  return {
    ...course,
    start_date: '',
    date_text: endDate ? `Available until ${endDate}` : '',
    metadata: {
      ...(course.metadata || {}),
      original_release_date: startDate,
      display_date_suppressed: true,
    },
  };
}
...
export function getCourseEligibility(course = {}, today = getTodayISO()) {
  ...
  if (startDate) {
    ...
    if (isSelfPacedOrEvergreen(course)) {
      return { eligible: true, reason: 'active self-paced course' };
    }
...
export function filterCurrentOrFutureCourses(courses = [], options = {}) {
  ...
  for (const course of courses) {
    ...
    if (decision.eligible) {
      rows.push(prepareCourseForPublication(course, today));
    } else {
      skipped.push({
