...
async function fetchExistingProvidersByName(supabaseAdmin, providerNames) {
  const existingRows = new Map();

  for (const providerNameChunk of chunkItems(providerNames)) {
    const { data, error } = await supabaseAdmin
      .from('providers')
...
export async function syncProviderCatalog(courses) {
  ...
  const providerRows = dedupeRowsBySourceKey(courses.map((course) => buildProviderRow(course)));
  const existingProviders = await fetchExistingProvidersByName(
    supabaseAdmin,
    providerRows.map((row) => row.name)
  );
  ...
  for (const row of providerRows) {
    const existing = existingProviders.get(normalizeText(row.name).toLowerCase());
    if (existing?.id) {
...
export async function getCatalogStats() {
  ...
  const { data: providersByName, error: providersByNameError } = await supabaseAdmin
    .from('providers')
    .select('name');

  if (providersByNameError) throw providersByNameError;

  return {
    providers: providerCount || 0,
    courses: courseCount || 0,
    sessions: sessionCount || 0,
    providerNames: (providersByName || []).map((row) => row.name).filter(Boolean),
  };
}
...
export async function getPublicCourseCatalog() {
  ...
  return allRows.map((row) => {
    ...
    sessions.sort((a, b) => {
      const aPriority = a.start_date ? (a.start_date >= new Date().toISOString().slice(0, 10) ? 0 : 2) : 1;
      const bPriority = b.start_date ? (b.start_date >= new Date().toISOString().slice(0, 10) ? 0 : 2) : 1;
