...
import { filterCurrentOrFutureCourses, getTodayISO } from './course-eligibility.js';
...
function buildSessionRow(course, courseId) {
  return {
    ...
    metadata: cleanValue({
      ...
      venue_address: course.venue_address || null,
      venue_latitude: course.venue_latitude ?? null,
      venue_longitude: course.venue_longitude ?? null,
      original_metadata: course.metadata || null,
    }),
  };
}
...
function isExpiredSession(session, today) {
  const effectiveDate = normalizeText(session.end_date || session.start_date).slice(0, 10);
...
export async function pruneExpiredCatalog(today = getTodayISO()) {
  const supabaseAdmin = getSupabaseAdmin();
  const datedSessions = await fetchAllRows(() =>
    supabaseAdmin
      .from('course_sessions')
      .select('id, course_id, start_date, end_date, format, schedule_text')
      .or(`start_date.lt.${today},end_date.lt.${today}`)
  );
  const expiredSessions = datedSessions.filter((session) => isExpiredSession(session, today));
  const affectedCourseIds = [...new Set(expiredSessions.map((session) => session.course_id).filter(Boolean))];

  for (const sessionChunk of chunkItems(expiredSessions.map((session) => session.id))) {
    const { error } = await supabaseAdmin.from('course_sessions').delete().in('id', sessionChunk);
    if (error) throw error;
  }

  if (!affectedCourseIds.length) {
    return { deletedSessions: 0, deletedCourses: 0 };
  }

  const remainingCourseIds = new Set();
  for (const courseIdChunk of chunkItems(affectedCourseIds)) {
    const { data, error } = await supabaseAdmin
      .from('course_sessions')
      .select('course_id')
      .in('course_id', courseIdChunk);
    if (error) throw error;
    for (const row of data || []) remainingCourseIds.add(row.course_id);
  }

  const orphanedCourseIds = [];
  for (const courseIdChunk of chunkItems(
    affectedCourseIds.filter((courseId) => !remainingCourseIds.has(courseId))
  )) {
    const { data, error } = await supabaseAdmin
      .from('courses')
    ...
    if (error) throw error;
    orphanedCourseIds.push(...(data || []).map((row) => row.id));
  }

  for (const courseIdChunk of chunkItems(orphanedCourseIds)) {
    const { error } = await supabaseAdmin.from('courses').delete().in('id', courseIdChunk);
    if (error) throw error;
  }

  return {
    deletedSessions: expiredSessions.length,
    deletedCourses: orphanedCourseIds.length,
  };
}
...
const PUBLIC_CURRENT_COURSE_SELECT = PUBLIC_COURSE_SELECT.replace(
  'course_sessions (',
  'course_sessions!inner ('
);

function currentSessionFilter(today = getTodayISO()) {
  return [
    `end_date.gte.${today}`,
    `and(end_date.is.null,start_date.gte.${today})`,
    'and(start_date.is.null,end_date.is.null)',
    'and(end_date.is.null,format.ilike.*Evergreen*)',
    'and(end_date.is.null,format.ilike.*On Demand*)',
    'and(end_date.is.null,format.ilike.*On-Demand*)',
    'and(end_date.is.null,format.ilike.*Self Paced*)',
    'and(end_date.is.null,format.ilike.*Self-Paced*)',
    'and(end_date.is.null,format.ilike.*Asynchronous*)',
    'and(end_date.is.null,schedule_text.ilike.*Release Date*)',
  ].join(',');
}

function mapPublicCourseRows(rows) {
  return (rows || []).map((row) => {
    const sessions = (row.course_sessions || []).map((session) => {
      const availabilityText = [row.category, session.format, session.schedule_text]
        .filter(Boolean)
        .join(' ');
      const suppressOldReleaseDate = session.start_date
        && session.start_date < getTodayISO()
        && (!session.end_date || session.end_date >= getTodayISO())
        && /evergreen|on[-\s]?demand|self[-\s]?paced|asynchronous|online course|release date/i.test(availabilityText);

      return suppressOldReleaseDate
    ...
    return {
      ...
      next_city: nextSession?.city || null,
      next_state: nextSession?.state || null,
      next_country: nextSession?.country || null,
      next_session_address: nextSession?.metadata?.venue_address || null,
      next_session_latitude: typeof nextSession?.metadata?.venue_latitude === 'number' ? nextSession.metadata.venue_latitude : null,
      next_session_longitude: typeof nextSession?.metadata?.venue_longitude === 'number' ? nextSession.metadata.venue_longitude : null,
...
export async function getPublicCourseCatalog() {
  ...
  while (true) {
    ...
    const { data, error } = await supabase
      .from('courses')
      .select(PUBLIC_CURRENT_COURSE_SELECT)
      .or(currentSessionFilter(), { referencedTable: 'course_sessions' })
      .range(start, end);
...
export async function getPublicCourseCatalogPage(page = 1, pageSize = 50, includeCount = true) {
  const supabase = getSupabasePublic();
  ...
  const { data, error, count } = await supabase
    .from('courses')
    .select(PUBLIC_CURRENT_COURSE_SELECT, includeCount ? { count: 'exact' } : undefined)
    .or(currentSessionFilter(), { referencedTable: 'course_sessions' })
