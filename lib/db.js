...
function isExpiredSession(session, today) {
  const endDate = normalizeText(session.end_date).slice(0, 10);
  ...
  const effectiveDate = normalizeText(session.start_date).slice(0, 10);
...
function isPublicSessionCurrent(session, today) {
  const endDate = normalizeText(session.end_date).slice(0, 10);
  if (endDate) return endDate >= today;
...
function mapPublicCourseRows(rows) {
  ...
  return (rows || []).flatMap((row) => {
    const sessions = (row.course_sessions || [])
      .filter((session) => isPublicSessionCurrent(session, today));
...
export async function getPublicCourseCatalogPage(page = 1, pageSize = 50, includeCount = true) {
  ...
  const { data, error, count } = await supabase
    .from('courses')
    .select(PUBLIC_COURSE_SELECT, includeCount ? { count: 'exact' } : undefined)
