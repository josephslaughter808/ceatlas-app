...
function parsePageCount($, baseUrl) {
  const lastHref = $('a.last.next').first().attr('href') || '';
  if (lastHref) {
    try {
      const lastPage = Number(new URL(lastHref, baseUrl).searchParams.get('pg'));
  ...
  const virtualCountMatch = ($.html() || '').match(/"_virtualItemCount":\s*(\d+)/);
  if (virtualCountMatch) {
    const total = Number(virtualCountMatch[1]);
    if (Number.isFinite(total) && total > 0) return Math.ceil(total / 25);
  }
...
function extractRowsFromPage($, pageUrl, config) {
  ...
  $('a[id$="_hlCourseTitle"]').each((_, link) => {
    ...
    rows.push(normalizeCourse({
      ...
      url,
      title,
      description: description || `${title} is listed in the ${config.provider} continuing education library.`,
      course_type: isSeries ? 'Series / Curriculum' : 'Online Course',
      format: 'Online',
      audience: config.audience || 'Dentists and Dental Team',
      ...
      metadata: {
        ...
        extracted_from: config.metadataSource,
      },
    }));
  });
...
export async function scrapeTownCE(config) {
  console.log(`   • Scraping ${config.provider}`);

  const firstPage = await loadTownHTML(config.startUrl);
  const totalPages = parsePageCount(firstPage, config.baseUrl);
  const rows = extractRowsFromPage(firstPage, config.startUrl, config);

  for (let page = 2; page <= totalPages; page += 1) {
    const pageUrl = `${config.baseUrl}/onlinece/viewall?pg=${page}`;
    const $ = await loadTownHTML(pageUrl);
    rows.push(...extractRowsFromPage($, pageUrl, config));
  }
  ...
  console.log(`   • Extracted ${deduped.length} ${config.provider} rows`);
  return deduped;
}
